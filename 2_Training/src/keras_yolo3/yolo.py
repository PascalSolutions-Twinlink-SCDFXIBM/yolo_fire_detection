...
def detect_video(yolo, video_path=os.getcwd(), output_path="",use_webcam=False):
    import cv2
    graph=[]
    fire=[]
    if use_webcam:
    ...
    while vid.isOpened():
        ...
        out_pred, image,count_mask,count_no_mask = yolo.detect_image(image, show_stats=False)
        result = np.asarray(image)
        graph.append(count_mask)
        fire.append(count_no_mask)
        # curr_time = timer()
        ...
        cv2.putText(
            result,
            text="People Detected: "+ str(count_mask),
            org=(3, 15),
    ...
    out.release()
#     yolo.close_session()
    return np.array(graph),np.array(fire)